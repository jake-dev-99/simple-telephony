...
object TelecomServiceRuntime {
    ...
    fun initialize(context: Context) {
        ...
        synchronized(this) {
            ...
            foregroundBridge = ForegroundChannelBridge(callStore)
    ...
    private fun ensureTracked(call: Call) {
    ...
    private fun dispatchCallUpdate(
        ...
        val record = CallSessionRecord(
            ...
            pendingEventCount = existingRecord?.pendingEventCount ?: 0,
            phoneNumber = call.detailsPhoneNumber(),
        ...
        if (callStore.getBackgroundHandlerConfig() != null) {
            callStore.enqueuePendingEvent(
                PendingCallEvent(
                    eventId = eventId,
                    callId = callId,
                    payload = payload,
